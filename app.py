...
def load_data():
    # 🔴 スプレッドシートのURL
    SPREADSHEET_URL = "https://docs.google.com/spreadsheets/d/1oomwgYozZgl9N9etkZkcqennHprrKX4h65VqqlsCV2E/edit?usp=sharing"
...
try:
    ...
    st.markdown(f"**基準日:** {latest_date} | **マクロ判定:** `{macro_data['地合い判定']}` (VIX: {macro_data['VIX']})")
    ...
    if not df.empty:
        ...
        def build_hover(row):
            return (f"<b>{row['銘柄']}</b><br>PER: {row['PER']} / ROE: {row['ROE']}<br>アクション: {row['💡総合投資アクション']}")
        ...
        fig = px.scatter(
            ...
            color_discrete_map={
                '買い': '#00CC96', '買い (強気)': '#00CC96', '打診買い': '#636EFA', '様子見': '#FECB52', '売り': '#EF553B'
            },
